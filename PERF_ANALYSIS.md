...
**Fix**: Load all 200 .class files into a single `ClassManager.define()` call with a 200-entry map, or better yet, bundle pre-built classes into a JAR and load via `URLClassLoader` — matching what executable-model does. Expected to bring UsingPreBuild from ~2438ms to ~200-400ms.
...
### 3. Batch Compilation in MVEL3 (HIGH — fixes PreBuild and NoPersist)

Collect all 200 generated Java sources and compile in a **single** `KieMemoryCompiler.compile()` call. Compiler startup/initialization overhead is currently paid 200 times. Expected to bring PreBuild/NoPersist DRLX within 1.5-2x of executable-model.

### 4. Cache ANTLR parse result (MEDIUM — fixes UsingPreBuild)
...
### 5. Share ClassManager across lambdas (MEDIUM)

Use one `ClassManager` for all lambdas in a build, enabling dedup cache hits and reducing per-instance overhead.
